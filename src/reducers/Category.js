...
export default (state = initialState, action) => {
  ...
  switch (action.type) {
    ...
    case 'SET_ITEMS':
      ...
      case 'PUSH_CATEGORY':
      {
        let initState = {...state, categories: [...state.categories, action.payload.id]};
    ...
    //  items2[0].categories[0].categories[0]
      ...
      for(let i = 1; i < initState.categories.length; i++)
      {
        ...
        console.log(res)
        let initState = {...state, categories: res};
        let items2 = {...initState};
        console.log(items2);
        items2 = items2.items[0];
        for(let i = 1; i < initState.categories.length; i++)
      ...
      case "ADD_TO_CART":
        ...
        {
          console.log(cart);
          let it = cart.find(i =>i.id === action.payload.id)
          let res = cart.filter(i =>  i.id !== action.payload.id)
